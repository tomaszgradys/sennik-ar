...
// تسوية عربية للمطابقة في البحث: إزالة أداة التعريف «ال» والسوابق الحرفية الشائعة
// (و/ف/ب/ك/ل + ال) بحذر، حتى يطابق «الكلب» رمز «كلب» والعكس. نعمل على السلاگ (بلا
// تشكيل، وقد سوّى slugify التاء المربوطة والألفات والياء). نُبقي جذعًا لا يقل عن 3
// أحرف كي لا نمسخ كلمات مثل «الله» أو «الآن» أو «الدم». لا نلمس اللواحق (جمع/تأنيث)
// لأن حذفها يدمج كلمات مختلفة خطأً (زيتون→زيت، قانون→قان).
export function arNorm(word: string): string {
  let s = word;
  if (s.length >= 6 && /^[وفبكل]ال/.test(s)) s = s.slice(1); // و/ف/ب/ك/ل قبل «ال»
  if (s.length >= 5 && s.startsWith("ال")) s = s.slice(2); // أداة التعريف «ال»
  return s;
}
